...
import { STATS, NOMBRE_STAT, IV_MAX, PRECIO_ELEGIR_SEXO, SEXOS } from './constantes.js';
...
function construir(nodoPedido, ctx, profundidad = 0) {
  ...
  if (nodo.naturaleza) {
    // Con Piedraeterna sólo queda un hueco de objeto, así que sólo se fuerza un IV:
    // el otro padre tiene que traer YA todos los IVs pedidos.
    const forzado = orden[0];
    const resto = nodo.stats.filter((s) => s !== forzado);
    ...
    nodo.explicacion =
      ...
      (resto.length
        ? `${resto.map((s) => NOMBRE_STAT[s]).join(', ')} tiene${resto.length > 1 ? 'n' : ''} que venir a 31 en los DOS padres.`
...
export function planear(objetivo, datos, { inventario = [], regionesDisponibles = [] } = {}) {
  ...
  // Tres pasadas, en este orden: el árbol de requisitos, el inventario encima, y
  // el reparto de sexos al final, que es lo único que depende de las dos cosas.
  const arbol = asignarSexos(
    asignarInventario(
      construir({ stats: pedidos, naturaleza: !!objetivo.naturaleza, rol: ROL.RAIZ }, ctx),
      ctx,
    ),
    objetivo,
  );
